...
void _blocked_fw_dependent_ph(const int blockId, size_t pitch, const int nvertex, float* const graph, int* const pred) {
    ...
    for (int u = 0; u < BLOCK_SIZE; ++u) {
        ...
        /**** MODIFIED TO MCP (MIN PLUS -> MAX MUL) ****/
        newPath = cacheGraph[idy][u] * cacheGraph[u][idx];
...
size_t _cudaMoveMemoryToDevice(const std::unique_ptr<graphAPSPTopology>& dataHost, float **graphDevice, int **predDevice) {
    ...
    size_t width_f = height * sizeof(float);
    size_t pitch;

    // Allocate GPU buffers for matrix of shortest paths d(G) and predecessors p(G)
    HANDLE_ERROR(cudaMallocPitch(graphDevice, &pitch, width_f, height));
    ...
    HANDLE_ERROR(cudaMemcpy2D(*graphDevice, pitch,
            dataHost->graph.get(), width_f, width_f, height, cudaMemcpyHostToDevice));
...
void _cudaMoveMemoryToHost(float *graphDevice, int *predDevice, const std::unique_ptr<graphAPSPTopology>& dataHost, size_t pitch) {
    ...
    HANDLE_ERROR(cudaMemcpy2D(dataHost->graph.get(), width_f, graphDevice, pitch, width_f, height, cudaMemcpyDeviceToHost));
 ...
 */
double cudaBlockedFW(const std::unique_ptr<graphAPSPTopology>& dataHost) {
    ...
    auto duration = (double) duration_cast<milliseconds>( stop - start ).count();
    ...
    HANDLE_ERROR(cudaDeviceSynchronize());
    _cudaMoveMemoryToHost(graphDevice, predDevice, dataHost, pitch);
    // std::cout <<  "time:"<< (double) duration << std::endl;
