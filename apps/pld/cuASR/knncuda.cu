...
__global__ void modified_insertion_sort_linear( float * dist,
                                                int *   index,
                                                int     width, //query number // N
                                                int     height,//ref numberr // M
                                                int     k){
    ...
    if (xIndex < width) {
        ...
        for (int i=1; i<height; ++i) {
            ...
            // Store current distance and associated index
            float curr_dist = p_dist[i*width];
            int   curr_index  = i;

            // Skip the current value if its index is >= k and if it's higher the k-th slready sorted mallest value
            if (i >= k && curr_dist >= p_dist[(k-1)*width]) {
                continue;
            }

            // Shift values (and indexes) higher that the current distance to the right
            int j = min(i, k-1);
            while (j > 0 && p_dist[(j-1)*width] > curr_dist) {
                p_dist[j*width]   = p_dist[(j-1)*width];
                p_index[j*width] = p_index[(j-1)*width];
                --j;
            }

            // Write the current distance and index at their position
            p_dist[j*width]   = curr_dist;
            p_index[j*width] = curr_index; 
        }
    }
}
...
__global__ void compute_sqrt_linear(float * dist, int width, int k){
    unsigned int xIndex = blockIdx.x * blockDim.x + threadIdx.x;
    unsigned int yIndex = blockIdx.y * blockDim.y + threadIdx.y;
    if (xIndex<width && yIndex<k)
...
bool knn_mxu(const float * ref,
    ...
    // Allocate global memory
    // half * ref_dev_half   = NULL;
    // half * query_dev_half = NULL;
    float * ref_dev   = NULL;
    float * query_dev = NULL;
    float * dist_dev  = NULL;
    int   * index_dev = NULL;

    // err0 = cudaMalloc((void**)&ref_dev_half,   ref_nb   * dim * size_of_half);
    // err1 = cudaMalloc((void**)&query_dev_half, query_nb * dim * size_of_half);
    err2 = cudaMalloc((void**)&ref_dev,   ref_nb   * dim * size_of_float);
    err3 = cudaMalloc((void**)&query_dev, query_nb * dim * size_of_float);
    err4 = cudaMalloc((void**)&dist_dev,  query_nb * ref_nb * size_of_float);
    err5 = cudaMalloc((void**)&index_dev, query_nb * k * size_of_int);

    
    // if (err0 != cudaSuccess || err1 != cudaSuccess || err2 != cudaSuccess || err3 != cudaSuccess || err4 != cudaSuccess || err5 != cudaSuccess) {
        if ( err2 != cudaSuccess || err3 != cudaSuccess || err4 != cudaSuccess || err5 != cudaSuccess) {        
        printf("ERROR: Memory allocation error\n");
        cudaFree(ref_dev);
    ...
    err0 = cudaMemcpy(ref_dev,   ref,    ref_nb * dim * size_of_float,    cudaMemcpyHostToDevice);
    err1 = cudaMemcpy(query_dev, query,  query_nb * dim * size_of_float,  cudaMemcpyHostToDevice);
    if (err0 != cudaSuccess || err1 != cudaSuccess) {
        printf("ERROR: Unable to copy data from host to device\n");
        cudaFree(ref_dev);
        // cudaFree(ref_dev_half);
    ...
    int ret = 0;
	ret = cuasr_plusmiussquare_srsgemm( query_nb,ref_nb,dim,
                                        query_dev, query_nb, 
                                        ref_dev, dim,
                                        dist_dev, query_nb,
                                        dist_dev, false,
    ...
    *kernel_time += elapsed_time + (toc.tv_usec - tic.tv_usec) / 1000000.;
    ...
    dim3 grid1(query_nb / 256, 1, 1);
    if (query_nb % 256 != 0) grid1.x += 1;
    modified_insertion_sort_linear<<<grid1, block1>>>(dist_dev, index_dev, query_nb, ref_nb, k);
    if (cudaGetLastError() != cudaSuccess) {
        printf("ERROR: Unable to execute kernel\n");
        cudaFree(ref_dev);
        // cudaFree(ref_dev_half);
    ...
    if (query_nb % 16 != 0) grid2.x += 1;
    if (k % 16 != 0)        grid2.y += 1;
    compute_sqrt_linear<<<grid2, block2>>>(dist_dev, query_nb, k);
    if (cudaGetLastError() != cudaSuccess) {
        printf("ERROR: Unable to execute kernel\n");
        cudaFree(ref_dev);
        // cudaFree(ref_dev_half);
    ...
    // Copy k smallest distances / indexes from the device to the host
    err0 = cudaMemcpy(knn_dist,  dist_dev,  query_nb * k * size_of_float, cudaMemcpyDeviceToHost);
    err1 = cudaMemcpy(knn_index, index_dev, query_nb * k * size_of_int,   cudaMemcpyDeviceToHost);
    if (err0 != cudaSuccess || err1 != cudaSuccess) {
        printf("ERROR: Unable to copy data from device to host\n");
        cudaFree(ref_dev);
        // cudaFree(ref_dev_half);
