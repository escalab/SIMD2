...
bool test(const float * ref,
    ...
    if  (nb_iterations > 1){
        ...
        //     printf("PASSED: %f / %f, %f / %f  ",precision_accuracy,min_accuracy,index_accuracy,min_accuracy);
        //     printf(" in %.0f us (dist kernel: %.0f us) (averaged over %3d iterations)\n", (elapsed_time / nb_iterations)*1000000, (elapsed_time_kernel/nb_iterations)*1000000,nb_iterations);
        // }
        // else {
        //     printf("FAILED: %f / %f, %f / %f  ",precision_accuracy,min_accuracy,index_accuracy,min_accuracy);
        //     printf(" in %.0f us (dist kernel: %.0f us) (averaged over %3d iterations)\n", (elapsed_time / nb_iterations)*1000000, (elapsed_time_kernel/nb_iterations)*1000000, nb_iterations);
        // }
...
int main(int argc, char **argv) {
    ...
    test(ref, ref_nb, query, query_nb, dim, k, knn_dist, knn_index, &knn_cuda_global,  "knn_cuda_global",  30);
