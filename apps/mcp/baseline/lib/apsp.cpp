 ...
 */
double apsp(const std::unique_ptr<graphAPSPTopology>& data, graphAPSPAlgorithm algorithm) {
    // std::function<double(const std::unique_ptr<graphAPSPTopology>&)> algorithms[] = {
    //         naiveFW,
    //         cudaNaiveFW,
    //         cudaBlockedFW
    // };
    // return algorithms[algorithm](data);
    return cudaBlockedFW(data);
}
